...
if ($pages <= 10) { // if total quantity of pages less or equal 10
	...
	for ($i=1; $i < $pages; $i++) {
		...
		<a class="page <?=$page_class?> <?=$active ? 'active_page' : '';?>" href="/<?=$page_file_name?>?page=<?=$i?><?=$table_prefix?><?=$search?>"><?=$i?></a>
	<?php }
	...
	if ($page < 7) { // if number of current page < 7
		...
		<?php // print last page ?>
		<a class="page <?=$page_class?>" href="/<?=$page_file_name?>?page=<?=$pages - 1?><?=$table_prefix?><?=$search?>"><?=$pages - 1?></a> <?php
	} elseif ($page >= $pages - 6) { // if number of current page more than total quantity - 6
		// print first page ?>
		<a class="page <?=$page_class?>" href="/<?=$page_file_name?>?page=1<?=$table_prefix?><?=$search?>">1</a>
