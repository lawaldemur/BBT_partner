...
if ($_POST['role'] == 'Команда')
	$books = $dbc->query("SELECT SUM(to_bbt) FROM sold $where AND `to_command_id` <> 0");
elseif ($_POST['role'] == 'Партнер')
	$books = $dbc->query("SELECT SUM(to_bbt) FROM sold $where AND `to_partner_id` <> 0");
else
	$books = $dbc->query("SELECT SUM(to_bbt) FROM sold $where AND `to_partner_id` <> 0");
	$books2 = $dbc->query("SELECT SUM(to_bbt) FROM sold $where AND `to_partner_id` = '0'");
...
elseif ($_POST['role'] == 'Партнер'){
	...
	<?php }
		...
		<div class="col-4">
			...
			<?php 
			//echo $period;
			//echo $b = ("SELECT SUM(to_bbt) FROM sold $where AND `to_partner_id` <> 0");
			foreach ($books as $money){
