...
if ($_POST['format'] != 'all')
	$where = 'WHERE `format` = \''.$_POST['format']."' AND to_partner_id = {$_POST['id']} AND ".$_POST['period'];
else
	$where = "WHERE to_partner_id = {$_POST['id']} AND ".$_POST['period'];

$result_on_d = $dbc->query("SELECT SUM(to_partner) FROM sold WHERE to_partner_id = {$_POST['id']} AND $date");
...
if ($_POST['graph'] == '0') {
	$result = $dbc->query("SELECT date, COUNT(id), SUM(to_partner) FROM sold $where GROUP BY date");
	$array = [];
	...
	foreach ($result as $item)
		$array[$item['date']][0] = $item['SUM(to_partner)'];
} elseif ($_POST['graph'] == '1') {
	$result = $dbc->query("SELECT WEEK(`date`,1) AS WEEK_NUM, DATE_SUB(`date`, INTERVAL WEEKDAY(`date`) DAY) 
		AS WEEK_MON, DATE_SUB(`date`, INTERVAL (WEEKDAY(`date`)-6) DAY) AS WEEK_SUN, COUNT(id), SUM(to_partner) 
	...
	foreach ($result as $item)
		$array[$item['WEEK_MON']][0] = $item['SUM(to_partner)'];
	...
	$result = $dbc->query("SELECT DATE_FORMAT(DATE_SUB(date, INTERVAL 0 MONTH), '%Y-%m-01') 
		...
		AS LastDayOfMonth, COUNT(id), SUM(to_partner)
	...
	foreach ($result as $item)
		$array[$item['FirstDayOfMonth']][0] = $item['SUM(to_partner)'];
}
